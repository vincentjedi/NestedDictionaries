...
def create_table_from_flattened_data(cursor, table_name, flattened_data):
    """
    Creates a PostgreSQL table dynamically based on the keys of the flattened data.
    """
    columns = []
    for key, value in flattened_data.items():
        ...
        columns.append(f"{key.replace('.', '_').replace('[', '_').replace(']', '_')} {col_type}")
...
def insert_flattened_data(cursor, table_name, flattened_data):
    """
    Inserts the flattened data into the table.
    """
    columns = [key.replace('.', '_').replace('[', '_').replace(']', '_') for key in flattened_data.keys()]
...
def main():
    ...
    # Flatten the data
    events_flattened = flatten_events(events_json)
    alerts_flattened = [flatten_events(alert) for alert in alert_json]

    # Create and insert for events table
    create_table_from_flattened_data(cursor, "events", events_flattened)
    insert_flattened_data(cursor, "events", events_flattened)

    # Create and insert for alerts table
    for alert_flattened in alerts_flattened:
        create_table_from_flattened_data(cursor, "alerts", alert_flattened)
        insert_flattened_data(cursor, "alerts", alert_flattened)
